-- Implementation of special skills (trigered when choosing to a skill in menu item.)
-- Each special skill is coded in WML object with id skill_idoftheskill
-- An attribut _level maybe added to abilities and specials when needed
-- Upgrading a skill should remove the old level and create a new one.
-- Some peculiar skills may also be implemented in Event combat or Abilities

---@type table<string, fun(lvl:integer, unit:unit)>
SPECIAL_SKILLS = {}

-- ---------------------------------- Vranken ----------------------------------
function SPECIAL_SKILLS.leeches_cac(lvl, unit)
    local value = Conf.special_skills.vranken.leeches_cac(lvl)
    unit:set_modification("skill_leeches_cac", T.effect {
        apply_to = "attack",
        range = "melee",
        T.set_specials {
            mode = "append",
            T.customName {
                id = "leeches",
                _level = lvl,
                value = value,
                name = _ "leeches " .. ROMANS[lvl],
                description = Fmt(_ "Regenerates %d%% of the damage dealt in offense and defense. Also works against undead", value)
            }
        }
    })
end

function SPECIAL_SKILLS.drain_cac(lvl, unit)
    local values = Conf.special_skills.vranken.drain_cac(lvl)
    local dmg, radius = values[1], values[2]
    unit:set_modification("skill_drain_cac", T.effect {
        apply_to = "attack",
        range = "melee",
        T.set_specials {
            mode = "append",
            T.drains {
                id = "drain_cac",
                _level = lvl,
                name = _ "drains",
                value = dmg,
                description = Fmt(_ "Regenerates %d%% of the damage dealt in offense and defense. Doesn't apply to undead", dmg),
                description_inactive = _ "Göndhul is too far away from Vranken.",
                T.filter_self {
                    T.filter_location {
                        radius = radius,
                        T.filter { id = "sword_spirit" }
                    }
                }
            }
        }
    })
end

---@param lvl integer
---@param unit unit
function SPECIAL_SKILLS.atk_brut(lvl, unit)
    local ratio = Conf.special_skills.vranken.atk_brut(lvl)
    local atk = unit:weapon("sword")
    unit:set_modification(
        "skill_atk_brut",
        T.effect {
            apply_to = "new_attack",
            name = "sword",
            range = "melee",
            type = "brut",
            damage = atk.damage * ratio / 100,
            number = atk.number,
            description = _ "ether sword " .. ROMANS[1],
            icon = "attacks/atk_brut.png",
            T.specials(atk:specials()),
        }
    )
end

---@param lvl integer
---@param unit unit
function SPECIAL_SKILLS.transposition(lvl, unit)
    local cd = Conf.special_skills.vranken.transposition(lvl)
    unit:custom_variables().special_skill_cd = 0
    unit:set_modification(
        "skill_transposition",
        T.effect {
            apply_to = "new_ability",
            T.abilities {
                T.customName {
                    id = "transposition",
                    _level = lvl,
                    name = _ "War link",
                    description = Fmt(_ "Vranken senses its sword spirit and may switch position with Göndhul, " ..
                        "no matter the distance between them. \n<b>%d</b> turn%s cooldown.", cd, cd > 1 and "s" or "")
                }
            }
        })

    UI.refresh_special_skills_menu()
end

-- ----------------------------------- Brinx -----------------------------------
function SPECIAL_SKILLS.def_muspell(lvl, unit)
    local value = Conf.special_skills.brinx.def_muspell(lvl)
    unit:set_modification("skill_def_muspell", T.effect {
        apply_to = "attack",
        T.set_specials {
            mode = "append",
            T.chance_to_hit {
                id = "def_muspell",
                _level = lvl,
                name = _ "muspell balance " .. ROMANS[lvl],
                description = Fmt(_ "Brinx learned how to better dodge muspellian weapons : %d%% bonus defense.", value),
                sub = value,
                apply_to = "opponent",
                T.filter_opponent { race = "muspell" }
            }
        }
    })
end

function SPECIAL_SKILLS.dmg_muspell(lvl, unit)
    local value = Conf.special_skills.brinx.dmg_muspell(lvl)
    unit:set_modification("skill_dmg_muspell", T.effect {
        apply_to = "attack",
        T.set_specials {
            mode = "append",
            T.damage {
                id = "def_muspell",
                _level = lvl,
                name = _ "muspell terror " .. ROMANS[lvl],
                description = Fmt(_ "Brinx deals %d%% bonus damage when facing a muspellian opponent.", value),
                multiply = 1 + value / 100,
                T.filter_opponent { race = "muspell" }
            }
        }
    })
end

function SPECIAL_SKILLS.fresh_blood_musp(lvl, unit)
    local value = Conf.special_skills.brinx.fresh_blood_musp(lvl)
    unit:set_modification("skill_fresh_blood_musp", T.effect {
        apply_to = "new_ability",
        T.abilities {
            T.customName {
                id = "fresh_blood_musp",
                _level = lvl,
                name = _ "Muspell strength",
                description = Fmt(_ "Brinx heals himself for %d HP when killing a muspellian", value),
            }
        }
    })
end

function SPECIAL_SKILLS.muspell_rage(lvl, unit)
    local value = Conf.special_skills.brinx.muspell_rage(lvl)
    unit:set_modification("muspell_rage", T.effect {
        apply_to = "new_ability",
        T.abilities {
            T.damage {
                id = "muspell_rage",
                _level = lvl,
                name = _ "Revenge",
                description = Fmt(_ "Brinx deals and takes %d%% bonus damage.", value),
                description_inactive = _ "There is no muspellian friend to anger Brinx.",
                apply_to = "both",
                multiply = 1 + value / 100,
                T.filter { T.filter_side { T.has_unit { race = "muspell" } } }
            }
        }
    })
end

-- ----------------------------------- Drumar ----------------------------------- --
function SPECIAL_SKILLS.wave_dmg(lvl, unit)
    local values = Conf.special_skills.drumar.wave_dmg(lvl)
    local dmg, nb_atk = values[1], values[2]
    unit:set_modification("skill_wave_dmg", T.effect {
        apply_to = "attack",
        name = "chill wave",
        increase_damage = tostring(dmg) .. "%",
        increase_attacks = nb_atk
    })
end

function SPECIAL_SKILLS.forecast_defense(lvl, unit)
    local def = Conf.special_skills.drumar.forecast_defense(lvl)
    unit:set_modification("skill_forecast_defense", AddDefenses(def))
end

function SPECIAL_SKILLS.slow_zone(lvl, unit)
    local intensity = Conf.special_skills.drumar.slow_zone(lvl)
    unit:set_modification("skill_slow_zone", T.effect {
        apply_to = "attack",
        name = "entangle",
        T.set_specials {
            mode = "append",
            T.customName {
                id = "slow_zone",
                _level = lvl,
                value = intensity,
                name = _ "slowing field " .. ROMANS[lvl],
                description = Fmt(_ "Decrease adjacent ennemies damages and movements by %d%%", intensity),
            }
        }
    })
end

-- enhance chilling special
function SPECIAL_SKILLS.bonus_cold_mistress(lvl, unit)
    local values = Conf.special_skills.drumar.bonus_cold_mistress(lvl)
    local dmg, nb_turn = values[1], values[2]
    local desc = Fmt(
        _ "Target takes <span weight='bold'>%d%%</span> bonus damage, when hit by cold attacks. Last <span weight='bold'>%d</span> turns.",
        dmg, nb_turn)
    unit:set_modification("skill_bonus_cold_mistress", T.effect {
        apply_to = "attack",
        name = "chilling touch",
        remove_specials = "status_chilled",
        T.set_specials {
            mode = "append",
            T.customName {
                id = "status_chilled",
                _level = lvl + 1,
                name = _ "chilling",
                description = desc
            }
        }
    })
end

-- ------------------------------------ Xavier ------------------------------------
---@param lvl integer
---@param unit unit
function SPECIAL_SKILLS.Y_formation(lvl, unit)
    local value = Conf.special_skills.xavier.Y_formation(lvl)
    unit:set_modification("skill_Y_formation", T.effect {
        apply_to = "new_ability",
        T.abilities {
            T.customName {
                id = "Y_formation",
                _level = lvl,
                name = _ "Back formation " .. ROMANS[lvl],
                description = Fmt(_ "Xavier sends its friends to distract the ennemy. When attacking in Y, " ..
                    "he has the time to perform <b>%d</b> additionnal attacks", value)
            }
        }
    })
end

---@param lvl integer
---@param unit unit
function SPECIAL_SKILLS.A_formation(lvl, unit)
    local value = Conf.special_skills.xavier.A_formation(lvl)
    unit:set_modification("skill_A_formation", T.effect {
        apply_to = "new_ability",
        T.abilities {
            T.customName {
                id = "A_formation",
                _level = lvl,
                name = _ "Wedge formation " .. ROMANS[lvl],
                description = Fmt(_ "Xavier is transcendanted by his friends. When fighting in A, " ..
                    "he gains <b>%d%% defense</b>.", value)
            }
        }
    })
end

---@param lvl integer
---@param unit unit
function SPECIAL_SKILLS.I_formation(lvl, unit)
    local value = Conf.special_skills.xavier.I_formation(lvl)
    unit:set_modification("skill_I_formation", T.effect {
        apply_to = "new_ability",
        T.abilities {
            T.customName {
                id = "I_formation",
                _level = lvl,
                name = _ "Spear formation " .. ROMANS[lvl],
                description = Fmt(_ "Xavier is transcendanted by his friends. When attacking in I, " ..
                    "he will gain <b>%d damages</b>.", value)
            }
        }
    })
end

---@param lvl integer
---@param unit unit
function SPECIAL_SKILLS.O_formation(lvl, unit)
    local value = Conf.special_skills.xavier.O_formation(lvl)
    unit:custom_variables().special_skill_cd = 0
    unit:set_modification("skill_O_formation", T.effect {
        apply_to = "new_ability",
        T.abilities {
            T.customName {
                id = "O_formation",
                _level = lvl,
                name = _ "Union formation",
                description = Fmt(_ "When grouping as 6, Xavier may disarm its opponent, removing its abilities " ..
                    "and weapon specials (<b>%d</b> turn(s) cooldown).", value)
            }
        }
    })
end

-------------------------------------- Morgane --------------------------------------
function SPECIAL_SKILLS.moves(lvl, unit)
    local new_costs = lvl == 1 and T.movement_costs {
        forest = 1,
        hills = 1,
        cave = 1,
        shallow_water = 2,
        swamp_water = 2,
        mountains = 2,
        frozen = 2,
    } or T.movement_costs {
        forest = 1,
        hills = 1,
        cave = 1,
        shallow_water = 1,
        swamp_water = 1,
        mountains = 1,
        frozen = 1,
    }
    unit:set_modification('skill_moves', T.effect {
        apply_to = "movement_costs",
        replace = true,
        new_costs
    })
end

function SPECIAL_SKILLS.limbes_defense(lvl, unit)
    local def = Conf.special_skills.morgane.limbes_defense(lvl)
    unit:set_modification("skill_limbes_defense", AddDefenses(def))
end

------------------------- Porthos -------------------------

function SPECIAL_SKILLS.pain_adept(lvl, unit)
    local percent = Conf.special_skills.porthos.pain_adept(lvl)
    unit:set_modification('skill_pain_adept', T.effect {
        apply_to = "new_ability",
        T.abilities {
            T.customName {
                id = "pain_adept",
                _level = lvl,
                name = _ "Pain adept " .. ROMANS[lvl],
                description = Fmt(_ "Porthos gains %d%% of his missing health as bonus damage (refreshed at each turn start).", percent)
            }
        }
    })
end

function SPECIAL_SKILLS.sacrifice(lvl, unit)
    local percents = Conf.special_skills.porthos.sacrifice(lvl)
    local adj, dist = percents[1], percents[2]
    local desc = dist == 0
        and Fmt(_ "Porthos shares %d%% of the damage inflicted to adjacent allies", adj)
        or Fmt(
            _ "Porthos shares %d%% of the damage inflicted to adjacent allies (%d%% for allies distant by 2 hexes).",
            adj, dist)
    unit:set_modification('skill_sacrifice', T.effect {
        apply_to = "new_ability",
        T.abilities {
            T.customName {
                id = "sacrifice",
                _level = lvl,
                name = _ "Sacrifice " .. ROMANS[lvl],
                description = desc
            }
        }
    })
end

-------------------------------------- Bunshop --------------------------------------

function SPECIAL_SKILLS.fangs_heal(lvl, unit)
    local heal_on_hit = Conf.special_skills.bunshop.fangs_heal(lvl)
    unit:set_modification('skill_fangs_heal', T.effect {
        apply_to = "attack",
        T.set_specials {
            mode = "append",
            T.heal_on_hit {
                id = "fangs_heal",
                _level = lvl,
                name = _ "heal " .. ROMANS[lvl],
                description = Fmt(_ "Bunshop heals himself by <b>%d</b> hp per hit.", heal_on_hit),
                value = heal_on_hit,
            }
        }
    })
end

--------------------------------------- Rymor ---------------------------------------
function SPECIAL_SKILLS.combat_shield(lvl, unit)
    local percent = Conf.special_skills.rymor.combat_shield(lvl)
    unit:set_modification('skill_combat_shield', T.effect {
        apply_to = "new_ability",
        T.abilities {
            T.customName {
                id = "combat_shield",
                _level = lvl,
                value = percent,
                name = _ "Shielder " .. ROMANS[lvl],
                description = Fmt(_ "At each turn, grants a shield of %d%% of his maximum hipoints to adjacent allies.", percent)
            }
        }
    })
end

function SPECIAL_SKILLS.atk_aura(lvl, unit)
    local nb_atk = Conf.special_skills.rymor.atk_aura(lvl)
    unit:set_modification('skill_atk_aura', T.effect {
        apply_to = "new_ability",
        T.abilities {
            T.attacks {
                affect_self = false,
                affect_allies = true,
                T.affect_adjacent {},
                add = nb_atk,
                name = _ "Attack aura " .. ROMANS[lvl],
                description = Fmt(_ "Rymôr grants his adjacent allies %d bonus attack.", nb_atk)
            }
        }
    })
end
