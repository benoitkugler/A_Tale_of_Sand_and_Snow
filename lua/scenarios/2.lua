...
local First_time = {
    {id = "see", name = "sighted", T.filter {side = 2}, T.lua {code = "ES.see_ennemy()"}}
}
...
function ES.kill()
    ...
    if dying.id == "leader" then
        wesnoth.fire(
            ...
            {
                speaker = "vranken",
                message = _ "At last ! Their leader has fallen, and the remaining bandits should flee. They won't be a threat anymore !"
            }
        )
        ...
        wesnoth.fire(
            ...
            {
                speaker = "jod",
                message = _ "<span style='italic'>(Shouting)</span> Sir ! Urgent message from the capital ! Seems like the Council needs you !"
            }
        )
...
function ES.turn1()
    ...
    wesnoth.fire(
        "message",
        {speaker = "drumar", message = _ "The Source is formal, Vranken, this should be an easy win !"}
    )
...
local function note_gondhul()
    ...
    wesnoth.fire(
        "objectives",
        {
            ...
            {"objective", {description = _ "Death of Frä Drumar.", condition = "lose"}},
            {"objective", {description = _ "Death of Bunshop.", condition = "lose"}},
            {"objective", {description = _ "Death of Göndhul.", condition = "lose"}},
...
function ES.see_ennemy()
    ...
    wesnoth.fire(
        "message",
        {speaker = "vranken", message = _ "<span style='italic'>(rubbing the pommel of his sword)</span>"}
    )
