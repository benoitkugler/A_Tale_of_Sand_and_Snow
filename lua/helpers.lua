...
-- Attention, prend en entrée une chaine sans <> significatif 
	local v = string.gsub(myString, "[<>]" , function(c) if c == "<" then return "{" else return "}" end end ) 
...
-- Attention, prend en entrée une chaine sans {} significatif 
	...
	local c = string.gsub(v, "[{}]" , function(c) if c == "{" then return "<" else return ">" end end ) 
	return c
end
...
function popup(title,message)
  ...
  T.grid { 
    ...
    local function preshow ()
        ...
        wesnoth.set_dialog_value("<span size='large' color ='#BFA63F' font_weight ='bold'>"..title.."</span>","the_title")
