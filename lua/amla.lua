...
-- Crée une attaque sur lemodèle de copy_ avec les mêmes dgts que l'attaque  à copier
local function update_copyobj(unit)
	...
	for spe  in H.child_range(u.__cfg,"attack") do
		if string.find(spe.name, "copy_") ~= nil then
			...
			wesnoth.add_modification(u,"object",{no_write="yes", {"effect" ,{ apply_to="attack" , name=spe.name , set_name=name, increase_damage=tocopy.damage,increase_attacks=tocopy.number , {"set_specials",H.get_child(tocopy,"specials")}  }}})
		
		end
	end
	wesnoth.extract_unit(u)
	local newu = purge_objet(u.__cfg)
...
setmetatable(db_lvl,{__index=function () return {} end})
    
local function update_lvl (unit)
...
-- Enlève les ALMA et rajoute l'alma default
