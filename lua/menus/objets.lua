...
function O.nouvel_objet(myObj)
    local obj = wesnoth.get_variable("objets_joueur")
    obj[myObj] = 0
    wesnoth.set_variable("objets_joueur",obj) 
end
-- enlève l'effet d'un objet et efface le tag object de l'unité

function O.remove(myId,myObj)
	local u = wesnoth.get_units{id = myId }[1]
	local obj_anti_effect = obj_DB[myObj]["contreff"]
	wesnoth.add_modification(u,"object",{{"effect",obj_anti_effect}},false)
	wesnoth.extract_unit(u)
	local newu = supp_id(u.__cfg,"trait",myObj)
...
-- applique l'effet d'un objet et le trait associé

function O.apply(myId,myObj)
	local u = wesnoth.get_units{id = myId }[1]
	local obj = obj_DB[myObj]
	local obj_effect = obj["eff"]
	local tr=  {id=myObj,name=obj.name,description = obj.description,{"effect",{} } }
	wesnoth.add_modification(u,"object",{{"effect",obj_effect}},false)
	wesnoth.add_modification(u,"trait", tr)	
...
local dialog = {
  ...
  T.grid { 
    ...
    T.row{T.column{ T.horizontal_listbox { id = "lobjets",T.list_definition{ T.row {T.column { T.toggle_panel { T.grid{ 
                        T.row{T.column{ border_size=5,border="all", T.image{id= "icone"} }},
                        T.row{T.column{ border_size=5,border="left,right,bottom", T.label{id= "name"} }}  }}}}}}}},
    T.row { T.column { T.spacer { id = "space2" ,height=10 } } },
    T.row { T.column{ T.grid{ T.row{   
                T.column { T.grid {         T.row { T.column { T.image { id = "owner_img"} }   },
                                            T.row { T.column { T.label { id = "owner_name"} }   } }},
                T.column { T.spacer { id = "space3",width=10 } } ,                          
                T.column { T.grid {         T.row { T.column { T.label { id = "objet_pres"} }   },
                                            T.row { T.column { T.image { id = "objet_img"} }   },
                                            T.row { T.column { border_size=5,border="all",T.label {characters_per_line=80, id = "objet_des"} }   } }     },
                T.column{ T.button{id="equip",tooltip=_"Click to change the object's owner",label=_"Equip"}}
   }}}} ,
    T.row { T.column { T.spacer { height=7 } } },
   T.row{ T.column{T.label{id="titre_heroes"}}} ,
    T.row { T.column { T.spacer { height=5 } } },
    T.row { T.column { T.horizontal_listbox { id = "lheroes",has_minimum=false,T.list_definition{ T.row {T.column { T.toggle_panel {T.grid{ 
                        T.row{T.column{ border_size=5,border="all", T.image{id= "icone"} }},
                        T.row{T.column{ border_size=5,border="left,right,bottom", T.label{id= "name"} }}  }}}}}} }
    },  
...
local function preshow ()
    ...
    wesnoth.set_dialog_value( "<span font_size = 'large' color ='#BFA63F' font_weight ='bold' >".._" Artifacts collected".."</span>", "titre")
    
    wesnoth.set_dialog_markup(true,"titre_heroes")
    wesnoth.set_dialog_value( "<span color ='#BFA63F' font_weight ='bold' >".._" Choose your hero".." :   </span>", "titre_heroes")
    ...
    local function affiche_list_heroes()
        ...
        for i,v in pairs(O.heroes) do
            ...
            if is_in_table(hero.id,O.objets) then
                wesnoth.set_dialog_value("<span color='#F4673E'>"..hero.name.."</span>","lheroes",i,"name") wesnoth.set_dialog_markup(true,"lheroes",i,"name")
            else
    ...
    function O.init()
        ...
        for i,v in pairs(O.objets) do
            O.min_length_description =  (O.min_length_description > string.len(tostring(obj_DB[i]["description"])) and string.len(tostring(obj_DB[i]["description"]))) or O.min_length_description
        end   
        ...
        for k,i in ipairs(sorted_keys(O.objets))	do
            ...
            if v == 0 then
                ...
                wesnoth.set_dialog_value("<span color='#F4673E'>"..objet.name.."</span>","lobjets",#(O.position_objets),"name") wesnoth.set_dialog_markup(true,"lobjets",#(O.position_objets),"name")
            end
        end
