...
GRAPH_PATH = "/home/benoit/.var/app/org.wesnoth.Wesnoth/data/wesnoth/1.17/data/add-ons/A_Tale_of_Sand_and_Snow/dev_tools/graph/images/"
...
def effet_texte(inColor, inImg):
    ...
    pdb.gimp_image_select_contiguous_color(
        inImg, CHANNEL_OP_REPLACE, layer, 1, 1)
    ...
    while keep:
        ...
        if (not _is_transparent(inImg, layer, xmiddle, y)) and _is_black(inImg, layer, xmiddle, y):
            ...
            pdb.gimp_image_select_contiguous_color(
                inImg, CHANNEL_OP_REPLACE, layer, xmiddle, y)
    ...
    pdb.gimp_image_select_rectangle(
        inImg, CHANNEL_OP_REPLACE, x1, y1, x2-x1, y2-y1)
    pdb.gimp_context_set_sample_transparent(True)
    pdb.gimp_image_select_contiguous_color(
        inImg, CHANNEL_OP_SUBTRACT, layer, x1, y1)
    pdb.gimp_edit_clear(layer)
    ...
    pdb.script_fu_layerfx_outer_glow(
        inImg, newlayer, inColor, 75, 0, 0, 0, 0, 5, 0, 1)
...
def change_num(image, inNumero):
    ...
    pdb.gimp_context_set_sample_transparent(True)
    pdb.gimp_image_select_contiguous_color(
        image, CHANNEL_OP_REPLACE, layer, 1, 1)
    ...
    pdb.gimp_context_set_sample_transparent(False)
    pdb.gimp_image_select_contiguous_color(
        image, CHANNEL_OP_REPLACE, layer, xthird, y)
    pdb.gimp_edit_clear(layer)
    ...
    if inNumero > 1:
        ...
        while keep:
            if not _is_transparent(image, layer, x, ymiddle):
                ...
                pdb.gimp_image_select_contiguous_color(
                    image, CHANNEL_OP_REPLACE, layer, x+1, ymiddle)
        ...
        pdb.gimp_image_select_rectangle(
            image, CHANNEL_OP_REPLACE, x1, y1, x2-x1, y2-y1)
...
def launch(arg_string):
    ...
    for i, text in enumerate(l):
        ...
        r, g, b, highlight_text, max_numero = int(r), int(
            g), int(b), int(highlight_text), int(max_numero)
