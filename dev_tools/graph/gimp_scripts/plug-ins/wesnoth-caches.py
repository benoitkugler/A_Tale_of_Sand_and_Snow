...
GRAPH_PATH = "/home/benoit/.var/app/org.wesnoth.Wesnoth/data/wesnoth/1.17/data/add-ons/A_Tale_of_Sand_and_Snow/dev_tools/graph/"
...
def cree_background(inLayer, inLayerF, Img):
    ...
    pdb.gimp_image_select_contiguous_color(
        Img, CHANNEL_OP_REPLACE, newlayer, 10, 10)
...
def load_caches(args):
    ...
    inColorFondR, inColorFondG, inColorFondB = int(
        inColorFondR), int(inColorFondG), int(inColorFondB)
    ...
    pdb.gimp_image_select_round_rectangle(
        baseimage, CHANNEL_OP_REPLACE, 0, 0, xmax, ymax, 35, 35)
    ...
    layerfleche = pdb.gimp_image_get_layer_by_name(
        baseimage, inNameLayerFleche)
